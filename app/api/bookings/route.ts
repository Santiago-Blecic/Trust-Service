...
export async function POST(request: Request) {
  ...
  const body=await request.json(); const hours=Number(body.hours); const slug=typeof body.slug==="string"?body.slug:"";
  ...
  const {data:service,error:serviceError}=await supabase.from("services").select("id,provider:providers!inner(verification_status)").eq("slug",slug).eq("is_active",true).eq("provider.verification_status","approved").single();
