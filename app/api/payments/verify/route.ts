...
export async function POST(request: Request) {
  ...
  try { await client.connect(); const response=await client.request({command:"tx",transaction:transactionHash}); const tx=response.result.tx_json; const provider=Array.isArray(booking.provider)?booking.provider[0]:booking.provider; const customer=Array.isArray(booking.customer)?booking.customer[0]:booking.customer;
