...
export default function AuthPage() {
  ...
  return <main className="shell"><nav className="nav"><a className="brand" href="/">proof<i>ly</i></a></nav><section className="details"><h1 style={{ fontSize: 48 }}>{mode === "signup" ? "Create your Proofly account" : "Welcome back"}</h1><p className="lead">Sign in with a unique name and password. Every account receives a self-custody XRP Testnet wallet; Proofly never stores its recovery seed.</p><form className="profile" onSubmit={submit} noValidate><label>{mode === "signup" ? "Your unique name" : "Name"}<br/><input required maxLength={40} value={name} onChange={event => setName(event.target.value)} style={{ width: "100%", padding: 12, margin: "8px 0 16px" }} placeholder="e.g. santi-blecic" autoCapitalize="none"/></label><label>Password<br/><input required type="password" autoComplete={mode === "signup" ? "new-password" : "current-password"} value={password} onChange={event => setPassword(event.target.value)} style={{ width: "100%", padding: 12, margin: "8px 0 16px" }}/></label><button className="button">{mode === "signup" ? "Create account" : "Sign in"}</button>{message && <p className="notice">{message}</p>}</form><p className="muted">Without an email address, a forgotten password cannot be recovered.</p><button className="back" onClick={() => { setMode(mode === "signup" ? "signin" : "signup"); setMessage(""); }}>{mode === "signup" ? "Already have an account? Sign in" : "New to Proofly? Create an account"}</button></section></main>;
}
