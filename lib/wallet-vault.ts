...
export async function storeWalletSeed(address: string, seed: string, password: string) {
  ...
  const encrypted = await crypto.subtle.encrypt({ name: "AES-GCM", iv }, key, new TextEncoder().encode(seed));
...
export async function loadWalletSeed(address: string, password: string) {
  ...
  const seed = await crypto.subtle.decrypt({ name: "AES-GCM", iv: fromBase64(record.iv) }, key, fromBase64(record.ciphertext));
